...
async function testPathTraversal() {
  ...
  try {
    ...
    const opencodeDir = join(import.meta.dir, "../plugins/beads-compound/opencode");
...
async function testTemplateInjection() {
  ...
  const geminiDir = join(import.meta.dir, "../plugins/beads-compound/gemini/commands");

  try {
    // Read a command that has template variables
    const content = await Bun.file(join(geminiDir, "beads-plan.toml")).text();
...
async function testFilePermissions() {
  ...
  const cortexDir = join(import.meta.dir, "../plugins/beads-compound/cortex");
...
async function testPlatformFlagValidation() {
  ...
  try {
    ...
    if (output.includes("Installing beads-compound for claude")) {
  ...
  try {
    ...
    if (!output.includes("Installing beads-compound for") || output.includes("claude")) {
...
async function testConversionOutputs() {
  ...
  const opencodeDir = join(import.meta.dir, "../plugins/beads-compound/opencode");
  const geminiDir = join(import.meta.dir, "../plugins/beads-compound/gemini");

  // Verify OpenCode outputs exist
  try {
    const commandsExist = await Bun.file(join(opencodeDir, "commands/beads-plan.md")).exists();
  ...
  try {
    const commandsExist = await Bun.file(join(geminiDir, "commands/beads-plan.toml")).exists();
  ...
  try {
    const content = await Bun.file(join(opencodeDir, "commands/beads-plan.md")).text();
    if (content.includes("Generated by beads-compound")) {
  ...
  const cortexDir2 = join(import.meta.dir, "../plugins/beads-compound/cortex");
  try {
    const commandsExist = await Bun.file(join(cortexDir2, "commands/beads-plan.md")).exists();
  ...
  try {
    const content = await Bun.file(join(cortexDir2, "commands/beads-plan.md")).text();
    if (content.includes("Generated by beads-compound")) {
...
async function testFormatCompatibility() {
  ...
  try {
    const cortexDir3 = join(import.meta.dir, "../plugins/beads-compound/cortex");
...
async function main() {
  ...
  console.log("Testing beads-compound multi-platform conversion...\n");
