...
function injectHeaderAfterFrontmatter(content: string, header: string): string {
  ...
  if (endIdx === -1) return header + content;
...
async function convertCommands() {
  ...
  await Promise.all(
    mdFiles.map(async (file) => {
      ...
      const content = raw
        .replace(/\.claude\/skills\//g, ".cortex/skills/")
        .replace(/ls \.claude\/skills\//g, "ls .cortex/skills/")
        .replace(/\.claude\/hooks\//g, ".cortex/hooks/");
...
async function convertSkills() {
  ...
  // Rebuild from scratch so stale generated skills never persist.
  await rm(outputDir, { recursive: true, force: true });
  await mkdir(outputDir, { recursive: true, mode: 0o755 });

  const skillDirs = await readdir(skillsDir, { withFileTypes: true });
  const coreSkills = skillDirs
    .filter((d) => d.isDirectory() && d.name !== "optional")
    .map((d) => d.name);
  const optionalRoot = validatePath(skillsDir, "optional");
  const optionalDirs = await readdir(optionalRoot, { withFileTypes: true });
  const optionalSkills = optionalDirs.filter((d) => d.isDirectory()).map((d) => `optional/${d.name}`);
  const skills = [...coreSkills, ...optionalSkills];

  // Process each skill directory in parallel
  await Promise.all(
    skills.map(async (skill) => {
      const skillDir = validatePath(skillsDir, skill);
      const skillName = skill.replace(/^optional\//, "");
      const outputSkillDir = validatePath(outputDir, sanitizeFilename(skillName));
      ...
      const header = `<!-- Generated by lavra v${PLUGIN_VERSION} -->
<!-- Source: ${skill}/SKILL.md -->
<!-- DO NOT EDIT - changes will be overwritten on next install -->

`;
      const withHeader = injectHeaderAfterFrontmatter(content, header);
