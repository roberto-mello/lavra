...
- "Rebase conflicts detected. Resolve conflicts, then run `git rebase --continue` and re-run /beads-ship."
...
Run a lightweight review focused on ship-blockers only. This is NOT a full /beads-review -- it checks for things that should never ship.
...
1. **Check for knowledge comments**
   ...
   ```bash
   bd show {BEAD_ID} | grep -cE "LEARNED:|DECISION:|FACT:|PATTERN:|INVESTIGATION:"
   ```
...
3. **Check for compound-worthy findings**
   ...
   If LEARNED or INVESTIGATION comments exist, note this for the summary -- the user may want to run /beads-compound later to extract reusable knowledge.
...
(or "No knowledge entries -- consider running /beads-compound")
...
- Run /beads-compound to extract reusable knowledge (if LEARNED/INVESTIGATION comments found)
...
Phase 4 is a safety net, not a code review. It catches ship-blockers only. For thorough review, use /beads-review before or after /beads-ship.
