...
<objective>
Report bugs encountered while using the beads-compound plugin by gathering structured information and creating a GitHub issue for the maintainer.
</objective>
...
```bash
# Get plugin version
cat ~/.claude/plugins/installed_plugins.json 2>/dev/null | grep -A5 "beads-compound" | head -10 || echo "Plugin info not found"
...
```bash
gh issue create \
  --repo rbm/beads-compound-plugin \
  --title "[beads-compound] Bug: [Brief description]" \
