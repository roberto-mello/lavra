...
---
name: report-bug
description: Report a bug in the lavra-compound plugin
argument-hint: [optional: brief description of the bug]
disable-model-invocation: true
---

<objective>
Report bugs encountered while using the lavra-compound plugin by gathering structured information and creating a GitHub issue for the maintainer.
</objective>
...
```bash
# Get plugin version
cat ~/.claude/plugins/installed_plugins.json 2>/dev/null | grep -A5 "lavra-compound" | head -10 || echo "Plugin info not found"
...
```bash
gh issue create \
  --repo rbm/lavra-compound-plugin \
  --title "[lavra-compound] Bug: [Brief description]" \
