...
```bash
# Count components
echo "Agents: $(find plugins/lavra-compound/agents -name '*.md' | wc -l)"
echo "Commands: $(ls plugins/lavra-compound/commands/*.md | wc -l)"
echo "Skills: $(ls -d plugins/lavra-compound/skills/*/ 2>/dev/null | wc -l)"

# Validate JSON
cat .claude-plugin/marketplace.json | jq . > /dev/null && echo "marketplace.json valid"
cat plugins/lavra-compound/.claude-plugin/plugin.json | jq . > /dev/null && echo "plugin.json valid"
```

## Step 2: Check for Uncommitted Changes

```bash
git status --porcelain plugins/lavra-compound/
```
