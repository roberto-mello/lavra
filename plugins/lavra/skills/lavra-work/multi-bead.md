# Multi-Bead Work Path

Internal reference for `/lavra-work` when multiple beads are being worked on. Dispatches subagents in parallel with file-scope conflict detection and wave ordering. Each subagent runs implement -> self-review -> learn. The orchestrator runs `/lavra-review` after each wave.

---

<phase name="gather" order="1">
...
<phase name="recall-config" order="6">
...
| `{EXTRA_INSTRUCTIONS}` | Empty for standard `/lavra-work` |
...
Task(general-purpose, "...filled prompt for BD-003...")
```
...
<phase name="verify" order="8" gate="must-complete-before-next-wave">
...
<mandatory>
After tests pass, run `/lavra-review` on the wave's changes. This is controlled by `workflow.review_scope`:

- `"full"` (default): invoke `/lavra-review` using the Skill tool. This is not optional.
- `"targeted"`: invoke `/lavra-review` if any bead in the wave is P0/P1 or contains architecture/security terms. Otherwise skip.

Wait for `/lavra-review` to complete before proceeding.
</mandatory>
...
For each bead completed in this wave that has a `## Validation` section, dispatch goal-verifier in parallel. Add `model: opus` when `model_profile` is `"quality"`. Skip beads with no Validation section. Only verify beads from this wave.
...
-- run in parallel for all qualifying beads
