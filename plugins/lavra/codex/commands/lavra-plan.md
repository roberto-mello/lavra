...
All checks are **warnings only** — they do not block submission. Ask user directly in chat (Codex-compatible) to ask whether to proceed or fix warnings first.
...
<handoff>
After creating the epic and child beads, use the **direct user prompt** to present these options:
