...
```bash
# Count agents
find plugins/lavra-compound/agents -name "*.md" | wc -l

# Count commands
ls plugins/lavra-compound/commands/*.md | wc -l

# Count skills
ls -d plugins/lavra-compound/skills/*/ 2>/dev/null | wc -l
```

Read all component files to get their metadata:

### Agents
For each agent file in `plugins/lavra-compound/agents/`:
...
### Commands
For each command file in `plugins/lavra-compound/commands/*.md`:
- Extract the frontmatter (name, description, argument-hint)
- Categorize as Workflow or Utility command

### Skills
For each skill directory in `plugins/lavra-compound/skills/*/`:
...
1. **`plugins/lavra-compound/.claude-plugin/plugin.json`**
...
cat plugins/lavra-compound/.claude-plugin/plugin.json | jq .

# Verify counts match
echo "Agents in files: $(find plugins/lavra-compound/agents -name '*.md' | wc -l)"
echo "Commands in files: $(ls plugins/lavra-compound/commands/*.md | wc -l)"
echo "Skills: $(ls -d plugins/lavra-compound/skills/*/ | wc -l)"
```
