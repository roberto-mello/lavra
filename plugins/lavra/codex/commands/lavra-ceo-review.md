...
Ask user directly in chat (Codex-compatible) to present three options:
...
**STOP.** direct user prompt once per issue. Do NOT batch. Do NOT proceed until user responds.
...
#### TODOS protocol
Present each potential TODO as its own individual direct user prompt. Never batch TODOs — one per question.
...
Identify at least 5 "bonus chunk" opportunities (<30 min each). Present each as its own direct user prompt. For each: what it is, why it would delight users, effort estimate. Options: **A)** Create a backlog bead **B)** Skip **C)** Build it now.
...
- **Stop-per-issue** -- One direct user prompt per finding with tradeoffs. Never batch issues.
...
<handoff>
After presenting the completion summary, use the **direct user prompt**:
