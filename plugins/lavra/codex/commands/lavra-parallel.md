...
```
If any found, Ask user directly in chat (Codex-compatible): "Found {N} beads left in_progress from a previous run. Reset to open?"
...
4. If no valid test command found: use direct user prompt to ask the user. Do NOT let workers self-discover test commands.
...
**If on the default branch**, Ask user directly in chat (Codex-compatible):
...
**When --teams mode:**
Present the plan once with direct user prompt including teams-specific parameters:
...
**When --ralph mode:**
Present the plan once with direct user prompt including execution parameters:
