...
```
If found, Ask user directly in chat (Codex-compatible): "Found {N} beads left in_progress from a previous run. Reset to open?"
...
**If on default branch**, Ask user directly in chat (Codex-compatible):
...
Present the plan with direct user prompt:
