...
<!-- Source: lavra-brainstorm/SKILL.md -->
...
Use the **direct user prompt** to ask questions **one at a time**. Keep asking until the picture is clear -- do not rush this phase.
...
Ask user directly in chat (Codex-compatible) to present a numbered list:
...
For each selected gray area, ask 3-4 targeted questions using **direct user prompt** (one at a time) to drive toward a decision.
...
Ask user directly in chat (Codex-compatible) to ask which approach the user prefers.
...
Ask user directly in chat (Codex-compatible) to present the proposed phases:
...
Ask user directly in chat (Codex-compatible) to present your recommendation with a brief rationale (2-3 sentences) and let the user confirm or pick a different mode.

**6.2 Force the hard questions:**

Based on the chosen mode, ask these questions using **direct user prompt** (one at a time):
...
Ask user directly in chat (Codex-compatible) to present next steps:
