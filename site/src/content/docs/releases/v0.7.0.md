...
The previous 150-line cap on bead descriptions incentivized compression, which stripped the context that prevents agent drift. Inspired by [Mario Zechner's observations](https://x.com/badlogicgames/status/2033798602372288947) on agent-driven coding: agents drift when specs are vague, not when specs are long. The bottleneck is review quality, not code generation speed.

The line cap is replaced with two new gates:

- **Completeness gate** -- every child bead must include What/Context/Decisions/Testing/Validation/Files/Dependencies sections with enough detail that the implementing agent makes zero judgment calls. If that takes 300 lines, fine. If 50, also fine.
...
Multi-bead mode defaults to parallel agent dispatch (up to `max_parallel_agents` concurrent). Running 3 agents concurrently means 3x the review debt -- and automation bias means those reviews become increasingly performative.
...
See [Security Model](../SECURITY.md#knowledge-system-injection-defense) for the full threat model and recommendations for collaborative projects (CODEOWNERS, audit practices, stealth mode).
...
- **`docs/SECURITY.md`** -- full threat model for config and knowledge injection defense
- **Expanded sanitization** -- Unicode bidirectional overrides, `[INST]`, null bytes, knowledge system parity with config defenses
