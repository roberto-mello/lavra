...
A `testing_scope` key was added in v0.7.0 for `/lavra-design` and `/lavra-plan`. When set to `"targeted"`, it scopes test cases to hooks, API routes, and complex business logic, skipping structural/render-only tests.

`/lavra-work` was not reading this key, so the setting only affected planning but not implementation. `/lavra-work` now reads `testing_scope` from `lavra.json` and applies it to deviation rule 2 (auto-adding tests for missing critical functionality).
