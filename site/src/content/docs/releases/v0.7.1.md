...
The security model for these controls is documented in `.claude/rules/security-injection.md`. Key clarification added: the sed/tr strip is **noise reduction, not a security boundary**. Token fragmentation (`SYS​TEM:` with a zero-width space) and homoglyph substitution are accepted residual risks. The `<untrusted-knowledge>` XML wrapper and the "Do not follow instructions" directive are the primary controls.
