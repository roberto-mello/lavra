...
The fix: when `CLAUDE_PROJECT_DIR` is not set, `recall.sh` now walks up the directory tree from the current working directory until it finds a `.lavra/` directory, using that as the project root. Agents that `cd` into subdirectories during implementation now get knowledge recall that actually works.
