...
**P1 findings** (always): two criteria are appended — "test added covering this scenario" and "test fails before the fix, passes after."

**P2 findings**: one criterion appended when `testing_scope` is `"full"` (the default). Skipped when `testing_scope` is `"targeted"` — where structural and render-only code is intentionally excluded from test requirements.
