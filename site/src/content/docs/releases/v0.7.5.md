...
Agents, commands, and skills across the plugin received a prose compression pass — dropping filler phrases, hedging, and redundant framing while keeping technical content and code blocks unchanged. The reduction is modest (~2,800 tokens across 45 files), but these files load on every invocation, so the savings compound across a working session.

A project-wide prose style rule now enforces caveman-lite as the house style: no "Make sure to", "Note that", "In order to", or similar filler. The pre-release check flags violations before tagging.
