...
`_kb_sync_jsonl` used a `tail -n +SKIP` shortcut to avoid re-importing entries already in SQLite. The skip offset was calculated as `DB_COUNT - 50`, where `DB_COUNT` came from the SQLite row count after the first-time bead-comment import.

The assumption: DB row count ≈ JSONL line count. It breaks immediately after a cold rebuild (`rm knowledge.db && kb_sync`), because the first-time path populates SQLite from `bd sql` comments before touching `knowledge.jsonl`. If the bead-comment import produced more rows than there are lines in `knowledge.jsonl`, `tail -n +N` overshot the end of the file and returned nothing. Every JSONL-only entry — including anything written by `/lavra-learn` — was silently skipped.

The optimization was saving sub-millisecond work (a PRIMARY KEY index lookup per row) at the cost of silent data loss. It's gone. `_kb_sync_jsonl` now reads the full file and relies on `kb_insert`'s existing key-level deduplication, which was already there for exactly this reason.

If you ran `/lavra-learn` and then rebuilt your knowledge DB, your curated entries may be missing from SQLite. Run `kb_sync` once after upgrading to recover them.
